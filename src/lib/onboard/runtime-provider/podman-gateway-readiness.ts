...
function classifyEndpointBinding(
  ...
  const loopback = new URL(expectedEndpoint);
  loopback.hostname = "127.0.0.1";
  const acceptedOrigins = new Set([expected.origin, loopback.origin]);
  if (!endpoints.length) return "unknown";
  return endpoints.every((endpoint) => endpoint !== null && acceptedOrigins.has(endpoint))
    ? "match"
    : "mismatch";
}
